...
public class ExpotToCWEXMLWizard extends Wizard implements IExportWizard {

	private ModelCatalogExportPage catalogPage;

	private Catalog initialSelection;

	public ExpotToCWEXMLWizard() {
		...
		if (catalog != null && targetFile != null && selectedFormat != null) {
			...
			try {
				getContainer().run(true, false, monitor -> {
					...
					new CWETransformer().exportCatalog(catalog, targetFile, selectedFormat);
					monitor.done();
				});
				return true;

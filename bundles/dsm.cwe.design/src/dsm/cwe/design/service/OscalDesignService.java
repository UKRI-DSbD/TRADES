...
package dsm.cwe.design.service;
...
import dsm.cwe.design.dialogs.ParameterValueSetterDialog;
import dsm.cwe.design.wizards.ConvertToCWEControlWizard;
import dsm.cwe.model.DocumentationComputer;
import dsm.cwe.model.CWECatalog.Catalog;
import dsm.cwe.model.CWECatalogCommon.Parameter;
import dsm.cwe.model.CWEMetadata.PartOwner;
import gov.nist.secauto.metaschema.datatypes.markup.MarkupLine;

public class CWEDesignService {

	/**
	 * Action used to export the selection {@link Control} into an CWE Catalog
	 * 
	 * @param controlToExport the list of control to export
	 */
	public static void exportControlToCWE(List<Control> controlToExport) {
		if (!controlToExport.isEmpty()) {
			ConvertToCWEControlWizard wizard = new ConvertToCWEControlWizard(controlToExport,
	...
	public static ExternalControl createControl(dsm.cwe.model.CWECatalog.Control control,
		...
		if (usedParameters.isEmpty()) {
			...
			// Keeps the entered value in the CWE model for next use
		}
	...
	private static Map<String, String> fillParameterValuesAndUpdate(List<Parameter> parameters, PartOwner element) {
		...
		// Update CWE library to keep entered value for subsequent use

...
public class CWEUtils {
	...
	public static void activateCWEViewpointIfMissing(Session session, IProgressMonitor monitor) {
		if (session.getSelectedViewpoints(true).stream()
				.noneMatch(v -> Activator.CWE_VIEWPOINT_NAME.equals(v.getName()))) {
			...
			List<String> existingViewpoints = session.getSelectedViewpoints(true).stream().map(Viewpoint::getName)
					.collect(toList());
			existingViewpoints.add(Activator.CWE_VIEWPOINT_NAME);

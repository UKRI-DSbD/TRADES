...
public class EmbeddedCatalogRegistry {
	...
	private String cveCatalogName = "cvec_v4.13";

	private EmbeddedCatalogRegistry() {
		nameToPath.put(nistCatalogName, "catalog/" + nistCatalogName + ".xml");
		nameToPath.put(cveCatalogName, "catalog/" + cveCatalogName + ".xml");
	}
	...
	public List<String> getAvailableCVELibraries() {
		return nameToPath.keySet().stream().filter(c -> c.startsWith("cve")).sorted().collect(toList());
	}

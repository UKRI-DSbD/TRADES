...
outputRoot.set('name', 'ATTACK')
outputRoot.set('id', 'ATT&CK')
threatOwner = ET.SubElement(outputRoot, 'threatOwner')
controlOwner = ET.SubElement(outputRoot, 'controlOwner')

folder_path = "C:\\temp\\Mitre"

# Constants
threatSRC = "MITRE ATT&CK"  # The source of the data from the threat ID
threatSRCID = "Attack"
...
for fileName in glob.glob(os.path.join(folder_path, 'T*.attck')):
    with open(fileName, 'r', encoding='UTF8') as myFile:
        ...
        threatTag.set('sourceID', threatSRCID)
        ...
        for div in divs:
            ...
            if attribute == 'ID:':
                ...
                tradesThreatID = threatSRCID + '_' + wordList[-1]  # Get the last word aka ID and
...
for fileName in glob.glob(os.path.join(folder_path, 'M*.attck')):
    with open(fileName, 'r', encoding='UTF8') as myFile:
        html_content = myFile.read()
        print(fileName)

        # ---------------------- control tag ------------------- #
        # Create new control tag
        controlTag = ET.SubElement(controlOwner, 'externals')

        html = BeautifulSoup(html_content, "html.parser")

        # Get the control name from file
        h1_tag = html.find('h1')
        if h1_tag:
            controlName = h1_tag.text.strip()
        else:
            print("No <h1> element found in the HTML document.")
            continue  # Skip to the next file

        # Create list of controls
        threatsList.append(controlName)

        # Set the name to the tag        
        controlTag.set('name', controlName)

        # Set the source of the control
        controlTag.set('source', threatSRC)
        controlTag.set('sourceID', threatSRCID)
        ...
        for div in divs:
            ...
            if attribute == 'ID:':
                # Generate the ID attribute
                tradesControlID = threatSRCID + '_' + wordList[-1]  # Get the last word aka ID and
                controlTag.set('ID', tradesControlID)  # Set it as an ID attribute

                # Generate the threat link from the threat ID and add it to the link
                controlLink = linkMitigationPrefix + wordList[-1]
                controlTag.set('link', controlLink)

            # If the current attribute is platform
            if attribute == 'Platform:':
                # Generate the threat Applicability
                controlTag.set('Applicability', div.text.split(':', 1)[1].strip())
